...
export function registerSdkCommands(program) {
    ...
    sdk
        ...
        .action((opts) => withTelemetry("sdk:generate", async () => {
        if (!opts.input || !opts.output) {
            throw new ValidationError("APPNEURAL requires input and output options");
        }
        await withSpinner("Generating APPNEURAL SDK", async () => generateSdkFromOpenApi(String(opts.input), String(opts.output)));
        logger.success(`APPNEURAL SDK generated at ${String(opts.output)}`);
    }));
    sdk
        .command("list")
        .description("List generated APPNEURAL SDKs")
        .action(() => withTelemetry("sdk:list", async () => {
